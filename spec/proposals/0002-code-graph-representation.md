...
4. **Build ② or stay ⓪-slice?** The first concrete consumer arrived — the
   call-chain navigation system (`docs/design/specs/geml-codemap/DESIGN-geml-code-graph.md`) — and it needed
...
5. **Tooling.** `tools/graph2geml.mjs` was the exploratory serializer
   ...
   deterministic emit → verify), P0 shipped per `docs/design/specs/geml-codemap/DESIGN-geml-code-graph.md`.
...
call-chain navigation system (`docs/design/specs/geml-codemap/DESIGN-geml-code-graph.md`, P0: the code-graph
...
(deterministic incremental emit). P1 (Joern) shipped 2026-07-03: on valkey src/, resolved cross-file calls went 12 → 23,235 (smoke gate 4/4, incl. the c->cmd->proc pointer call honestly listed as unresolved). P2 shipped same day: the history-engine unique-key diff optimization (2.9s -> 0.1s at 14k units, unblocking .gemlhistory at graph scale), build --history (per-code-change architectural history; a removed edge rewrote and committed exactly the forward + backlink documents, and geml revert restored one node), and a zero-dep MCP wrapper (resolve_name / open_symbol / get_backlinks). Remaining: visibility (.private) from CPG data, partition-quality re-measure; incremental emit machinery stays deferred per the design trigger (full recompute 1.2s << 30s).
