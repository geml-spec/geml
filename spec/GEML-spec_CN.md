...
- **数据来自别处** —— 表格可以不写内联正文，改为声明数据来自哪里。对于 `table` 块，`src=` 与 `data=` 是同一属性的两种写法（不同于 `diagram` 或 `embed` 块中两者语义不同，见附录 B.3），同时出现是错误。三种目标形态按同一规则解析：
  ...
  （错误）。内联仍是默认。
- **合并单元格** —— 在块的属性上通过 `span` 声明为 `span="r<R>c<C>:<H>x<W>[,...]"`，其中 `<R>` 和 `<C>` 是合并区域左上角单元格的 1-based 行列号，`<H>` 和 `<W>` 是占据的行数（高度）与列数（宽度）（例如 `span="r2c1:2x1"` 表示从第 2 行第 1 列开始，高 2 行宽 1 列）。多个合并规则用逗号分隔。画出的 ASCII 表格必须（MUST）保持网格状；在渲染时，被合并范围覆盖的单元格会被忽略。
- **计算列** —— `compute` 列出一条或多条 `列名 = 表达式` 公式，以 `;` 分隔。每条
  表达式对每个数据行求值一次，运算符限 `+ - * / ( )` 与一元 `-`（`*`/`/` 优先级高于
  `+`/`-`，左结合）。当遇到空值或非数值单元格时，行级公式计算会将其作为 `0` 处理以保证公式完成；而在执行聚合函数时，除 `count` 会统计所有非空单元格的数量外，其余函数（如 `sum`、`avg`）都会跳过非数值单元格（不计入总和或平均值分母）。列按表头名引用（名字含空格用引号，如
