...
| `[[#id]]` | navigation, automatic link text | §5.2 |
...
|-----|------------|--------|------------|
| `data=#id` | `diagram` (`geml-chart`) | a `table` block in the same document | §7.1 |
| `of=#id` | `output` | a `code` block in the same document | §3 |
| `src=` | `table` | external data (`csv`/`tsv`): document-relative path or `http(s)` URL | §6 |
...
| Code | `` `code` `` | `=== code` |
| Math | `$…$` | `=== math` |
| Embedding — render a target in place | `![alt](src)` (media) | *(none in this version)* |
...
projected at the document's foot. Projection of GEML *block content* —
rendering another block or section in place — has no syntax in this version of
the language.
