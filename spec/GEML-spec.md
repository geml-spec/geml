...
| Fragment identifier | a block `id`, or a prose-run address derived from two of them (§4) |
...
it is written as a GEML reference or as a URL. It may also denote a **prose run**
— the text between two blocks, which bears no `id` of its own — by the address §4
derives for it. A processor MUST resolve both, and the reason is §8.2(5): an
...
- An `embed` block stands for content that lives elsewhere: `src=` names a
  ...
  heading's whole section (the heading itself and all subsequent blocks up to, but not including, the next heading of the same or higher level, or the end of the document); a fragment naming a prose run (§4) selects that run. `src=` is reference-checked like any other reference
...
- **A prose run has an address, derived from the two things around it.** The text
  ...
  A **prose run** is a maximal run of adjacent content that is neither a typed
  block nor a heading. Its **container** is the innermost heading whose section
  ...
  | no *P* — the run opens the container | `C-before-N` |
  | no *N* — the run closes the container | `C-after-P` |
  | neither | none: the container holds nothing else, so `C` already names it |

  Written with the ids themselves, so `#pub-before-cmd` is the run between the
  ...
  cannot spell it differently. Each form names BOTH ends of its run, which is what
  ...
  (§8.2(5)) instead of quietly naming a shorter run.

  Three rules complete it:
  1. An address needs its anchors to HAVE ids. A run beside an anonymous block, or
  ...
  2. A declared `id` always wins. A processor resolves a fragment against the
     ...
     `{#pub-before-cmd}` shadows the run rather than competing with it.
