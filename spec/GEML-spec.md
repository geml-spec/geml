...
Six attribute keys carry references; all of them are validated (Appendix A):
...
| `src=` | `data` | the block's external content: a `.json`/`.jsonl` file, document-relative path or `http(s)` URL (render-time) | §3.2 |
