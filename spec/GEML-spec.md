...
| Fragment identifier | a block `id`, or a prose address derived from two block ids (§4) |
...
it is written as a GEML reference or as a URL. It may also denote **prose** — the text between two blocks, which bears no `id`
of its own — by the address §4 derives for it. A processor MUST resolve both, and the reason is §8.2(5): an
unresolved reference is an error, so a fragment one processor understands and
another does not is the same document building in one and failing in the other.

*Note (non-normative):* the two extensions distinguish a document's **role**,
not its format. A `.gemlhistory` sidecar is a complete, valid GEML document: it
declares its vocabulary in `=== meta` like any other
(`profile = "geml-history/v1"`) and validates with no diagnostics. The separate
name exists so tools can route the two apart — a directory walk that collects
documents takes `.geml` and leaves the sidecar, whose keyframe mirrors the live
file and whose blobs hold superseded text, so sweeping it in answers every
search twice and with content that is no longer there. Tooling that recognizes
`.geml` for highlighting, rendering or parsing can treat `.gemlhistory`
identically; neither name is a vocabulary declaration, which §8.6.2(2) rules out
and the sidecar's own `profile` line makes unnecessary.
...
An item's content starts on the marker line and MAY continue on **continuation
lines**: a non-blank line directly below the item (no blank line between) that
is not an item line, not a `%%` comment line, and whose indentation is
...
Multi-paragraph list items are not part of GEML — a blank line always ends the
item, and a continuation after a blank line is ordinary block content. Rich
item content belongs in a typed block (§3).
...
type           = TYPE-NAME ;                  (* narrower than NAME; see below *)
...
TYPE-NAME      = ASCII-LETTER , { ASCII-LETTER | DIGIT | "-" | "_" } ;
                 (* the block type is the ONE name a document does not derive
                    but chooses: it keys the type registry, is typed into
                    commands, and reaches generated output as an identifier. So
                    it is ASCII and starts with a letter, where NAME — which
                    ids, classes and attribute keys use — is deliberately
                    wider. Unicode here would buy two encodings of one name
                    (NFC/NFD) and invisible confusables in a registry key, with
                    no matching need: nothing forces a type name the way a
                    non-Latin heading forces a non-Latin id. *)
```
...
- **Prose between two blocks has an address, derived from the two of them.** The text
  between two blocks bears no `id`, so nothing can name it and a document
  assembled out of references silently loses it. The derivation below closes that,
  and is **normative** for the same reason the heading derivation above is: a
  reference has to name the same content in every implementation.
  ...
  **Prose**, here, is a maximal stretch of adjacent content that is neither a
  ...
  | | address |
  |---|---|
  | *P* and *N* both present | `P-between-N` |
  | no *P* — the prose opens the container | `C-before-N` |
  | no *N* — the prose closes the container | `C-after-P` |
  | neither | none: the container holds nothing else, so `C` already names it |

  Written with the ids themselves, so `#pub-before-cmd` is the prose between the
  heading `#pub` and the block `#cmd`. The relation word is **determined by the
  structure, not chosen**, so a run has exactly one address and two processors
  cannot spell it differently. Each form names BOTH ends of the stretch, which is what
  makes an edit loud: insert a block between `#cmd` and `#verify` and
  `#cmd-between-verify` no longer names contiguous prose, so it stops resolving
  (§8.2(5)) instead of quietly naming a shorter stretch.

  Three rules complete it:
  1. An address needs its anchors to HAVE ids. Prose beside an anonymous block, or
     directly in a document body, has no address; it is still content, merely not
     referenceable.
  2. A declared `id` always wins. A processor resolves a fragment against the
     declared ids first and these addresses only after, so a block someone names
     `{#pub-before-cmd}` shadows the prose rather than competing with it.
  3. **These addresses are matched, never parsed back.** A processor derives the
     set for a document and looks a fragment up in it; it does not split an address
     to recover its anchors. So an id containing `-before-`, `-after-` or
     `-between-` cannot make one ambiguous, and no escaping is needed to stay
     inside NAME (§4).
...
be admitted. For the types it admits — and only those — a vocabulary also
declares how their bodies are read (§8.6.2 rule 4).
...
A vocabulary that declares a body mode is declaring what its **own** types
hold. It may not restate the body mode of a type this specification registers —
`text` is `flow`, a `data` block's is its format engine's — because that would be
redefining a name it does not own, which is the last entry above. What decides
whether a construct belongs here or in this specification is therefore not a
property of its body but a question of ownership: a construct every reader of
...
3. MUST treat a declared name it does not recognize as admitting nothing, and
   MUST report `unrecognized-vocabulary` naming it. The declaration is not an
   error — the document is valid, and it is the processor's view of it that is
   incomplete, in a way the reader is now told about rather than left to
   discover. A processor that resolves an `=== embed` (§3) MUST report the
   target's unrecognized declaration too, at the embedding block: §3 parses the
   target as a document in its own right, so the host renders those blocks raw,
   and the host is the document the reader is looking at. It MUST report this
   only for a target **this document itself names**: a document reached solely
   through another document's embeds is not one this document has any
   relationship with, and naming it — its path, or the vocabulary it declares —
   would disclose a dependency of the target that the host never took content
   from. This is the line
   between two kinds of diagnostic — one about what this PROCESSOR cannot do,
   which follows the content wherever its effect shows, and one about a
   DOCUMENT's own fault, which stays with the document that has it. **Which
   vocabularies a processor recognizes is implementation-defined**, and a
   processor that recognizes none is conformant.
4. MUST NOT let admission change the set of addressable units, except as the
   vocabulary's declared body modes require and only in a processor that
   recognizes it. A processor that does NOT recognize a vocabulary reads every
   type it would have admitted as `raw` — the body §8.2(6) gives an unknown type
   — and has reported rule 3's diagnostic, so the difference between the two
   readings is announced rather than silent. Admission never changes the address
   a document's recognized blocks carry, nor any address outside the admitted
   types' bodies.

Rule 3's report is what makes rule 4's exception safe to grant. A processor
that cannot read a vocabulary's bodies says so, so a reader never mistakes a
partial view for the whole one, and an address that resolves under one processor
and not under another is accompanied by the reason. §3.2 already works this way
for a RESERVED `data` format a processor ships no engine for: the body stays raw,
`data-format-no-engine` says why, and the value tree that format would have
produced is not addressable in that processor. A vocabulary's bodies are the same
situation with the same remedy.

§8.4's suite is unaffected either way, because it is stated over the document
model and diagnostics were never part of it — rule 1 is already entirely about
diagnostics differing with what a processor recognizes. What keeps a document
safe to edit across a boundary is not this rule but §3: every document is parsed
under its own `=== meta`, so block extraction, block replacement and `=== embed`
read a target under the target's own declaration, and content moved between
documents cannot change meaning in transit.
