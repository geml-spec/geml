...
| `data=` | `diagram` (`geml-chart`) | where the data comes from, in the same three forms a table's `src=` takes: a data file (`csv`/`tsv`), `#id` naming a table block in this document, or `doc.geml#id` naming one in another. A data file stands for the anonymous table it describes. | §6, §7.1 |
