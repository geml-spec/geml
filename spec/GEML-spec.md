...
- **Punctuation** here is any character in a Unicode punctuation (`P*`) or symbol
  (`S*`) category — not merely ASCII. `“`, `）` and `，` are punctuation like `"`,
  `)` and `,` are, so `“*(foo)*”` emphasizes exactly as `"*(foo)*"` does. (The
  `\` escape of §5.1 is the opposite: it applies to ASCII punctuation only,
  because no non-ASCII character is GEML syntax to begin with.)
