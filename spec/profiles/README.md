...
| `geml-media/v1` | types `media-asset`, `media-clip`, `media-text` (`media-text` is prose) and their attribute keys; `.gen-log` on a `data` block | [geml-media-profile.md](geml-media/geml-media-profile.md) · [中文](geml-media/geml-media-profile_CN.md) | — |
