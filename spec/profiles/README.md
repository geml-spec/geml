...
| `geml-agent/v1` | draft | types `agent-vars`, `agent-state`, `agent-transition`, `agent-snapshot`, `agent-refused`; `initial`, `final`, `pause`, `tools`, `vars`, `rollback-on-error` on `agent-state`; `from`, `to`, `requires`, `approval` on `agent-transition`; `rev`, `state`, `cause`, `parent`, `hash`, `at`, `call`, `from`, `restores` on `agent-snapshot`; `rev`, `at`, `tool`, `call` on `agent-refused` | [geml-agent-profile.md](geml-agent/geml-agent-profile.md) · [中文](geml-agent/geml-agent-profile_CN.md) | `geml-agent check\|snapshot\|verify\|export\|init` (package `@geml/agent-runtime`) |
