...
样式表自己那个 `=== meta` 里的**每个键都是一个记号**。`style-rule` / `style-state` /
`style-screen` / `style-frame` 的任何属性里，`{{key}}` 在装载样式表时换成那个键的值。
...
为什么需要它，是量出来的：本 profile 附的整页复刻样式表里，`#d1d9e0` 出现 21 次、
`#59636e` 14 次、`#1f2328` 12 次——改一次边框色要动 21 个块。CSS 用自定义属性回答这个
问题已经十年了；这里没有对等物，样式表到整页规模就没法维护。

写法就是核心 GEML 自己的 `{{key}}`（规范 §4）。核心只在**流文本**里代换，属性里不动；
这里是本 profile 把同一个引用延伸到自己的文档放内容的地方。同一套语法、同一种失败方式，
没有新东西要学。
...
一样，所以 `{{profile}}` 会展开成 `geml-style/v1`——无害，不值得为它开一个特例。
...
`width` `max-width` `visible` `underline`。别的内含词写在部件规则上报 `style-unknown-attribute`（warning）
...
```
<type>? (#id)? (.class)* ([key] | [key=value])*      单个简单选择器——各段顺序无关
*                                                    任意节点（只能整步）
...
词汇正好是 §4 自己那套——类型、`#id`、`.class`、属性存在、属性等值——加一个组合子。
简单选择器的几段**无序**，和规范说属性无序是同一件事（`#id`、`.class`、`key=val` 是
推荐的书写顺序，不是规则）：`text#a.x`、`text.x#a`、`text[k=v].x#a` 是同一个选择器。
`.class` 和 `[key]` 是两个**永不相遇**的命名空间——类不是同名属性，所以 `.x` 命中
`{#a .x}`、`[x]` 命中旗标 `{#a x}`，互相都够不着。
...
**继承由宿主决定，本 profile 不描述它。** `box` 是平的：每个绑定带的就是 §4 裁决到它
身上的那几个词，视图模型里没有任何一处说子节点会继承父节点的 `color`。真实发生的是宿主
