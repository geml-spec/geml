...
- Status: **draft**, and its `media-text` type depends on [GEP-0013](../../proposals/0013-prose-body-for-vocabularies.md) (draft): until a vocabulary may declare a prose body, that type is a divergence from §8.6.2 rule 4, not a licensed extension. The vocabulary below is registered in the reference
  ...
  ([`playground/geml-media-ep01/`](../../../playground/geml-media-ep01/README.md));
...
## 3. `media-clip` — one cut on the timeline
...
| `dur` | sources with no intrinsic duration | how long a still, or a piece of prose, occupies the timeline |
...
- The **primary track** (`meta.primary`, default `video`) is **sequential**:
  ...
  `crossfade` are `transition-dur`, `fade` does not overlap). The first cut
  starts at 0.
- A cut's length is `out - in`, or `dur` for a source with no intrinsic
...
## 5. The generation log — `data {.gen-log format=jsonl}`
...
| `media-dur-required` | error | a source with no intrinsic duration and no `dur` |
...
## 8. What this profile does not admit
