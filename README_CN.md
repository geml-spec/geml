...
- **参考实现 + CLI** —— [`geml-parser/`](geml-parser/)。把文档解析为**文档模型 JSON**，有错误则以非零码退出。
  ...
  npm test
  ```
- **浏览器扩展** —— [`integrations/geml-viewer/`](integrations/geml-viewer/)，在本地（`file://`）与网络上渲染 `.geml`：带计算列的表格、作为内联 SVG 的 `geml-chart`、Mermaid 图、KaTeX 公式，以及作为横幅显示的构建期诊断。
...
examples/              一个示例 .geml 文档（+ CSV 数据），可自行渲染
tools/                 附加工具（graph2geml —— 已被继任，留作 GEP-0002 实证）
assets/                Logo 与品牌资产
```
