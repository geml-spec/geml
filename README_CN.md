...
- **工具链之路——写 tree-sitter 语法。** 一份语法同时点亮 **Neovim、Helix、Zed** 三家。[设计简报](integrations/tree-sitter/)已备好，语法还没有人写。
- **生态之路——写 Pandoc reader / writer。** 一次接入，GEML 就进入 Pandoc 已经服务的每一条流水线。

再加一句常设的挑衅：如果「为什么不直接用 Markdown」在你看来答案很明显——**不论哪个方向**——我们宁愿听你说出来，也不要你安静地同意。
