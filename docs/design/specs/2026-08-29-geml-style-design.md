...
#### 13.4i 链接的下划线：宿主别剥，加一个 `underline`

上一条留了个尾巴：`.geml-page a { color: inherit; text-decoration: none }`。整页布局下宿主替所有页面
decide 了「链接不带颜色和下划线」。量下来后果比想的重：这条规则盖住整页里**所有**链接，包括 `embed` 进来的
那篇文章 —— 正文里的链接实测是 `rgb(31,35,40)` 无下划线，和正文的字一模一样，看不出是链接。没有人要求过这个。

**改法**：宿主什么都不剥，HTML 的默认（蓝色带下划线）立着；去掉它是外观，由样式表说。为此加一个内含词
`underline=yes|no`，块和行内部件都收。

名字上排掉了两个候选。**不叫 `text-decoration`**：那是 CSS 的简写，管线型、线样式、线颜色、粗细四样，
借了名字只兑现一丝，正是 §12.4 说的「借 CSS 的词却换意思」那类陷阱。**不叫 `text-link`**：内含词命名的是
**属性**，作用在谁身上是**选择器**的事（`text#nav link`），把目标编进名字会是唯一一个这么干的词，而且仍然
没说清它设的是什么。

复刻页因此只多一条规则，而且比原先设想的更短：

```
=== style-rule {#links match="* link" color="#0969da" underline=no}
===
```

`*` 是任意节点，所以 `* link` 是**语料里的每一个链接**，连借来那篇文章里的也算；外壳各块自己的颜色规则是它的
真超集，各自覆盖。一句话说完「这一页的链接长什么样」。

**顺带补上一个真缺口**：`* link` 第一次跑的时候对正文没生效。原因是 `borrowedFor` 把借来的块直接画出去，
没给它们戴 `geml-placed geml-b-<地址>` 的外壳，于是样式表为它们求出了绑定、生成了 CSS，却没有元素可落 ——
**样式表管不到自己嵌进来的那篇文档**。一页的正文就是那份文档，这说不过去。现在借来的每个块都戴上自己的 class
和 `data-doc`，规则落得上了。
...
### 13.13 与 CSS 横向对照挖出来的三处（2026-09-11）

把 geml-style 和 CSS 逐维比了一遍（选择器、层叠、属性、取值、摆放、状态、at-规则、
继承、出错哲学、样式表是什么）。结论是八个维度上它是 CSS 的受限子集，两个维度
（歧义即错误、样式表决定摆放）刻意偏离且理由成立。对照挖出三处要动的，记在这里。

#### 13.13.1 记号：`meta` 的键 + 属性里的 `{{key}}`

**量出来的问题**：整页复刻样式表 104 块里，`#d1d9e0` 抄了 21 遍、`#59636e` 14 遍、
`#1f2328` 12 遍、`#ffffff` 10 遍。改一次边框色要动 21 个块。CSS 自定义属性回答这个
问题已经十年，这里没有对等物。

**决定**：样式表 `meta` 的每个键都是记号，四种 style 块的**任何属性值**里的
`{{key}}` 在装载期代换。没有新块类型、没有新语法——复用核心 §4 的 `{{key}}`
（核心只在流文本里代换，属性里不动，所以这一层自己做）。

四条边界：一遍代换不递归；按**文件**算（`embed` 进来的规则用它自己那份 `meta`，
与核心对借来内容的规定一致）；悬空是 **error**（新诊断码 `unknown-token`）；
**整个值恰好是一个记号时带上类型**——`hide-below="{{col}}"` 配 `col = 1012` 要得到
数字 1012，否则 `sticky` / `hide-below` 这类数值域的内含词永远喂不进记号。

**没选的**：新块类型 `style-tokens`（多一个块类型换不来别的）；放行 CSS 的
`var(--x)` 透传（`var(--x)` 本来就过得了 `safeCssValue`，但没有地方**定义**它，
等于把问题推给宿主的静态表——那正是这一轮要往回收的东西）。

**落地**：`tokensOf` / `expandTokens` / `sheetBlocks` 在 `style-resolve.ts`；
装载路径两处入口（宿主文档自己的 children、`expandEmbeds` 的跨文档分支）。
演示样式表已改写：8 个颜色 + 2 个页面常量各只出现一次，**视图模型与改写前逐字节相同**。

#### 13.13.2 简写与单边：源序漏洞堵上

**量出来的问题**：§4 的招牌是「没有源序兜底」，但裁决是按**属性名**做的，而
`border` 和 `border-left` 是两个名字——它们从不相遇，两条都活着，按写入顺序进同一个
`box`，再按 `Object.entries` 的插入顺序发成 CSS。实测同样两条规则只换文件里的先后：

| 文件里的先后 | 诊断 | 页面结果 |
|---|---|---|
| `#one`（`border`）在前 | 无 | 左边框被去掉 |
| `#two`（`border-left`）在前 | 无 | 左边框还在 |

零诊断。这正是 `geml add --before` 能悄悄改掉渲染结果的那个场景。

**决定**：同一层、同一 `when` 组里的两条**不同**规则，一条写 `border`、另一条写四个
单边之一 → `ambiguous-rule`。三处放行：写进同一条规则（那里的先后是作者写的，
`geml set` 换的是整块）；跨层（层是声明出来的顺序，规则按层收集，低层先进 box）；
几个单边之间（互不覆盖）。`border-radius` 不在这一族。

**没选的**：(b) 发 CSS 前按「简写在前、单边在后」排序——等于偷偷引入一条优先级规则，
而且当更具体的那条写的是简写时排反；(c) 去掉简写 `border` 只留四个单边——演示样式表里
`border=` 用了 16 次，成本远大于收益。(a) 最贴合已有哲学：不确定就不猜。

#### 13.13.3 继承：写明「未定义，由宿主决定」

**问题**：视图模型的 `box` 是平的，规范里既没说继承、也没说不继承。实际行为完全由
宿主发出的 CSS 决定——换一个不发 CSS 的宿主（画 canvas、排 PDF），同一份样式表渲染
结果会变，而一致性面抓不到。这是唯一一处「反直觉」是因为没想清楚，不是因为想清楚了。

**决定**：写进 profile §10，**明确为未定义、由宿主决定**，并说清代价（两个都合规的
宿主可能渲染成两样）和用法（要求到哪都一致的样式表，把词写在它真正想要的块上）。

**没选的**：为 28 个词各裁一遍「继承/不继承」。那是重新发明 CSS 的继承表，
而目前没有消费者需要它——等有了第二个非 CSS 宿主再说。

### 13.14 页面与模板分开（2026-09-11）

**观察**：`page.geml` 226 行，其中 225 行是页面框架（顶栏、仓库导航、文件树、面包屑、
提交行、四个菜单、工具条），只有最后一行 `=== embed {#doc src=…}` 说"看哪份文档"。
它其实是一个**页面模板**，带一个变量。

**决定**：拆成两份，`template.geml`（模板，223 行）+ `page.geml`（装配单，13 行）。

**零新机制** —— 这是这条决定成立的全部理由。`borrowedDocs` 把页面里**每个**
`embed src=` 的文档拉进语料；`byAddr = nodesByAddress(corpus)` 是整个语料的地址表；
槽位解析出来的是 `{doc, block}` 对。所以样式表的 `slots="text#brand"` 本来就够得到
另一份文档里的块，只是没人走过这条路。样式表**一个字没改**。

于是"直接写"和"用模板"是同一件事的两端，不是两个特性：把块写进 `page.geml` 就是
直接写（拆开之前就是那样，现在照样合法），写两个 `embed` 就是用模板。

**实测等价**：拆开前后整页渲染的 DOM **逐字符相同**（56310 字符，83 处 `icons/` 引用
两边一致），诊断 0/0，CSS 规则集合相同（只有发出的顺序随语料顺序变了，而每条规则
的选择器互不相同，顺序无意义）。

**模板放在 demo 根目录**，不放 `_index/`：借来文档里的相对 `src`/`href` 是按**那份
文档自己的 URL** 重定基的（`transclude.js` 的 `rebase`），模板一旦下沉到 `_index/`，
41 个 `icons/…` 就得改成 `../icons/…`。而且 `_index/` 的身份是"viewer 自己去发现的
样式入口"，模板是你手写 `src=` 指过去的东西，不是一类。

**顺带修的一个洞**：`embed {#template}` 没有任何槽位摆它，`unplaced` 会把它算成漏摆
（viewer 打一行 console.info）。但它的活儿是把文档带进语料，不是被摆 —— 它带进来的块
要么被摆了、要么自己会出现在那张单子上。所以：**一个 `embed`，只要它借的文档在语料里，
就不算漏摆**。

**没做**：给 `set` 加 `--attr` / `#doc.src` 之类的属性级写入（见 §13.15）。

### 13.15 为什么没有 `#doc.src`

拆完之后"换一份文档"就是改 `page.geml` 里一行 `src=`，于是自然会问：能不能
`geml set page.geml '#doc.src'`？

**现状**：`geml set` 的粒度已经比块细 —— GEP-0011 的坐标能写到表格单元格
（`#tbl[2]["port"]`）和 `data` 值树的节点（`#cfg[":server"][":port"]`），两者实测都
能写回。但坐标是 **body-only**：`#doc["src"]` 会被拒绝，理由是"`embed` 里面没有可寻址
的单元 —— 坐标需要 table 或 data 块"。head 上只有 `--head`，**整行替换**。

**`.` 不能用**：`.` 在这门语言里是**类**。块可以写 `{#a .x}`，profile §3 的选择器就是
`text#a.x`。核心 CLI 的选择器今天还不认 `.class`（`#a.src` 报 "no block with id
`a.src`"），但那是**还没加**，不是这个位置空着 —— 占掉它就等于永久放弃
`geml get f.geml '.todo'`。`@` 同理已被内容地址（`=== type@<hex>`）占了。

**也不加 `--attr`**（作者 2026-09-11 明确否掉）：把开关和数据混进一个 flag，而且它只
服务 `set`，`get` / `find` 那边还是够不着，一个地址语法多个动词的格局就破了。

**结论**：head 属性级寻址的位置留白，等有真实需求再定语法。真要写，今天的办法是整行：

```sh
printf '=== embed {#doc src="../../docs/MANIFESTO.geml"}' \
  | geml set page.geml '#doc' --head --in -
```

它有护栏 —— 规范化 id、发现 id 丢了拒写、发现文档会坏掉拒写（实测两条都会拦）。
