...
| 外部实体 | `SYSTEM "file:///etc/passwd"` —— XXE 的来源 | **没有这个机制** | ❌ |
| 跨文档包含 | XInclude | `other.geml#id` 引用，构建期解析，范围由 `--root` 界定 | 🔁 |
...
| XML：外部实体 / XXE | **没有实体机制**。`{{key}}` 只从本文档 `meta` 取值 |
| XML：实体递归 / billion laughs | **单次展开**，`a = "{{b}}"` 取出来是字面 `{{b}}`（实测） |
