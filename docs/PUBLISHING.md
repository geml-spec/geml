...
| "Open VSX token + namespace" | "Cursor / Windsurf / VSCodium / Antigravity — they do NOT read the VS Code Marketplace" | "NOT SET UP YET. Needs an Eclipse Foundation account; a signed Publisher Agreement; `ovsx create-namespace geml -p <token>`. No `ovsx` dependency and no token exist in this repo." |
...
- **Lands at TWO marketplaces from one package.** The VS Code Marketplace serves
  VS Code; **Open VSX** (open-vsx.org) serves the editors that cannot use it —
  Cursor, Windsurf, VSCodium and Antigravity all resolve extensions there. Same
  `.vsix`, same publisher name, two accounts and two tokens.
...
- **Open VSX is NOT set up in this repo yet.** No `ovsx` dependency, no token, no
  namespace — searching the tree for `ovsx`/`open-vsx` returns nothing. The
  one-time cost is an Eclipse Foundation account plus a signed Publisher
  Agreement; until it is paid, every Cursor and Antigravity user is on whatever
  version they side-loaded.
- **Confirm.** The VS Code Marketplace version · the Open VSX listing version,
  separately · install it and open a `.geml` file.
...
| "One `.vsix` has two marketplaces" | "Cursor and Antigravity users sit on an old version while the VS Code listing is current — they read Open VSX and never see it" | "nothing — publish the SAME packaged file to both with `-i`" |
