...
| "VS Code Marketplace publisher + PAT" | "`vsce publish`" | "NOT APPLIED FOR YET — there is no VS Code Marketplace listing, so plain VS Code has nothing to install from. Manual, outside this repo." |
| "Open VSX token" | "Cursor / Windsurf / VSCodium / Antigravity — they do NOT read the VS Code Marketplace, and this is the ONLY listing the extension has" | "DONE — Eclipse Foundation account, signed Publisher Agreement, namespace `geml` created. The token is not stored in this repo: `ovsx publish` is run by hand with `-p`." |
...
- **Lands at ONE marketplace today, and it is not the obvious one.**
  [**Open VSX**](https://open-vsx.org/extension/geml/geml) — namespace `geml`,
  extension `geml` — is live and is the ONLY channel: Cursor, Windsurf, VSCodium
  and Antigravity all resolve extensions there. **The VS Code Marketplace account
  is not applied for yet**, so plain VS Code users have no listing to install
  from at all. The design is still one `.vsix` for both, two accounts and two
  tokens; only one of the two exists.
- **Version** in `package.json` and `package-lock.json` (twice).
- **How.** Package ONCE, then publish that one file twice — both CLIs take
  `-i, --packagePath`, verified against `vsce` and `ovsx` help:
...
- **Watch for.** Do NOT run `vsce publish` and `ovsx publish` without a package
  path: each would build its own `.vsix`, and the two marketplaces would carry
  different bytes under one version number. The parser must be built before
  packaging, or the webview build fails on a missing `dist/`. The bundle is
  deliberately NOT committed here (~8 MB), so there is no staleness to check and
  nothing for CI to guard. Both registries refuse a duplicate version.
...
- **Confirm.** `curl -s https://open-vsx.org/api/geml/geml` names the version and
  the download count · install it and open a `.geml` file. Open VSX indexes a
  submission with a delay, so the API can still answer with the previous version
  for a few minutes after a successful publish. Nothing to confirm on the VS Code
  Marketplace until the account exists.
