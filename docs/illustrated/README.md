...
**Measurement baseline lives here, not on the pages.** Every output a page shows
was produced by the repository's own build — `node geml-parser/dist/geml.js` —
and the pages no longer each name a parser version: one page re-measured on a
newer parser than its neighbour used to leave a set of contradictory
provenances, and a version stamped on a page nobody re-ran is a claim rather
than a record. The **Status** column below is where a page's currency is stated;
a page whose claims have not been re-measured since a parser change says so
there.
...
|---|---|---|---|---|
| 1 | shared fence and attribute rules · `meta` · `math` · `note` · `text` | [01-simple-blocks.html](01-simple-blocks.html) | [_CN](01-simple-blocks_CN.html) | written |
| 2 | `code` · `data`, one `src=` route syntax | [02-code-data.html](02-code-data.html) | [_CN](02-code-data_CN.html) | written |
| 3 | `table` and `view` (§6.1) | [03-table-view.html](03-table-view.html) | [_CN](03-table-view_CN.html) | written |
...
| Page · board | Spec | The parser actually |
|---|---|---|
...
|---|---|---|
| ~~3 · 18, 4 · 15~~ **closed** | GEP-0012 (`view`) | *Was:* never mentions charts, so a chart over a computed column would have no source. §7 now settles it in the normative text — "`data=` may name a `view` as readily as a `table`, and for a **derived** column it must" — with `data=#fy25-report y=FY` as the worked case |
