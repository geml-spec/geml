...
这个仓库对外发七样东西，走六条版本轨道，其中**三样把解析器打包了一份拷贝进去**，
...
- **落到** VS Code Marketplace。
- **版本**在 `package.json` 与 `package-lock.json`（两处）。
- **怎么发。** 在 `integrations/vscode` 手工 `vsce publish`。它的
  `vscode:prepublish` 会跑 `compile` 和 `build:webview`，而后者是
  `npm --prefix ../geml-viewer run build:vscode` —— 解析器就是在这一步被打包进去的。
- **注意。** 打包前必须先构建解析器，否则 webview 构建会因为找不到 `dist/` 而失败。
  这里的 bundle **刻意不提交**（约 8 MB），所以没有陈旧问题，CI 也没有什么要守。
- **发后确认。** Marketplace 上的版本 · 装上并打开一个 `.geml` 文件。
...
- **落到**两个地方：Logseq marketplace（经镜像仓库
  `geml-spec/logseq-plugin-sync-vault-with-geml` 的 release），以及 npm 上的
  watcher `@geml/logseq-sync`。
- **版本**在 `package.json`、`plugin/package.json` 与 `package-lock.json`（三处）。
  插件与 watcher **同版本一起发**；MAJOR 跟随它对话的那个 Logseq 大版本。
- **怎么发。** 把 `integrations/logseq/` **原样**镜像进那个独立仓库 —— 镜像仓库的根
  就是这个目录 —— 提交信息用
  `Sync Vault with GEML — mirror of geml-spec/geml integrations/logseq @ <sha>`，
  推送，**然后**才在那边打 `v<x.y.z>`。镜像仓库自己的 `publish.yml` 会构建插件并
  挂上 marketplace zip。
...
- **发后确认。** `gh release view v<x.y.z> -R geml-spec/logseq-plugin-sync-vault-with-geml`
  里资产名为 `logseq-plugin-sync-vault-with-geml-v<x.y.z>.zip` · 镜像仓库的
  `plugin/package.json` 已是新版本。
...
6. **升并发布携带它的产物**，各走各的轨道：viewer 打 tag、vscode 用 `vsce`、dsh 用
   `npm publish`。claude 与 codex 插件**已经上线了** —— 合并落地的那一刻就是发布。
7. **Logseq 只在它自己的代码变了时才发**。它的依赖是范围，新解析器不用发版就能到。
