...
- **`geml-history/v1`** — the `.gemlhistory` sidecar's own vocabulary
  (`revision`, `keyframe`, `blob`, and their attribute keys) is declared like
  any other application layer, and `geml history save` writes
  `profile = "geml-history/v1"` into the meta it produces. Until now every
  history file this project wrote reported its own blocks as
  `unknown-block-type` — 333 occurrences across the repo's seven sidecars, a
  file format defined by a sibling spec and generated by this very tool.
  Sidecars written before this are fixed by their next save, or by adding the
  one meta line; the revision chain is untouched either way
  (`geml history verify` passes on all seven).

### Changed
- **Profile names are prefixed `geml-`**, so a vocabulary this project ships is
  recognizable at a glance: `codemap/v1` is now **`geml-codemap/v1`**, joining
  `geml-style/v1` and `geml-history/v1`. Renaming costs nothing today — the
  profile mechanism landed after 1.8.8 and has never been in a published
  release. A document declaring the old name gets no vocabulary and warns; the
  fix is the new name, or a rebuild for generated documents.
