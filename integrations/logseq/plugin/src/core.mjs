// Plugin-side logic for GEML Sync, kept free of @logseq/libs so the tests can
...
export function formatStatus(raw) {
  if (!raw) {
    return "GEML Sync: no watcher status yet — is `geml-sync --watch --signal …` running?";
  }
  let s;
  try {
    s = JSON.parse(raw);
  } catch {
    return "GEML Sync: status file is not valid JSON.";
  }
  if (s.ok === false) {
    return `GEML Sync: last sync FAILED at ${s.at} — ${s.error}`;
  }
  ...
  return `GEML Sync: last sync at ${s.at} — ${parts.join(", ")}.`;
}
