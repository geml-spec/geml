...
2. **Detect the language(s) — NEVER ask the user.** (Steps 2–3 are the
   ...
   | `build.sbt` / `.scala` | **Not auto-detected — no build job**; hand the build a SCIP index: `--adapter scip --raw index.scip`. Scala indexing needs the LAST scip-java that had it, `com.sourcegraph:scip-java_2.13:0.12.3` (0.13, 2026-07, dropped Scala; Maven Central keeps 0.12.3, and the `v0.12.3` GitHub release ships a launcher + `.bat`). Compile with SemanticDB first (Scala 3 `-Xsemanticdb`; Scala 2.13 the `semanticdb-scalac` plugin; sbt `semanticdbEnabled := true`; Metals writes it already), then run IN the project root: `scip-java index-semanticdb --output index.scip <dir holding META-INF/semanticdb>`. Precise tier for calls — cross-file high, overloads resolved, trait dispatch → `candidates` — with three gaps to SAY SO about: the index has no `enclosing_range`, so caller attribution degrades to nearest-preceding definition and every method spans one line; Scala 3 SemanticDB records `new X(…)` as a type reference, so constructor calls are invisible; the converter is frozen upstream |
   | `build.zig` / `build.zig.zon` / `.zig` | tree-sitter fallback. Auto: `geml codemap build --root <proj>` detects it. Explicit: `GEML_SRC=<abs-src> GEML_OUT=<abs-raw> GEML_LANG=zig npx -y -p web-tree-sitter@0.25.10 -p tree-sitter-wasms@0.1.13 node <pkg>/codemap/treesitter-export.mjs` → `--adapter treesitter --raw <raw>` (first run fetches ~54 MB into the npx cache). **Heuristic tier — SAY SO**: no type information, so every edge is `resolution: heuristic`, never `high`. Same-file calls, `@import`-followed paths (aliases and re-exports included), struct namespaces and `self.` resolve `medium`; a method call on a value (`c.send()`) is matched by name only — unique → `low`, 2–8 same-named → `candidates`, more → `#unresolved` with the count. `test "…" {}` blocks yield no nodes or edges; `comptime`/`anytype`/function-pointer calls are invisible. Files the grammar cannot parse (syntax newer than tree-sitter-zig) are counted in the build output and their rows are partial — report that number |
