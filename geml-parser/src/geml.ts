...
const PARSER_VERSION = "1.3.1";       // reference implementation; keep in sync with package.json
...
const SUBHELP = {
  ...
  codemap: `usage: geml codemap build  [--root <repo>]   # auto-detect languages, run the indexer(s), and merge into one codemap (--root defaults to the current directory)
       geml codemap build  (--db <graph.db> | --adapter joern|scip|sql --raw <in>)+ [--root <repo>] [--out .geml-code-graph] [--container module|dir|file] [--lang <JAVASRC|NEWC|…>] [--joern <path>] [--sql-dialect <hive|postgres|…>] [--history [-m msg]]
