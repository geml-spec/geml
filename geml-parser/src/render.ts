...
.cg-search-row { display:flex; gap:10px; align-items:baseline; width:100%; text-align:left; padding:5px 9px; border:0; border-bottom:1px solid var(--bd); background:none; color:var(--fg); cursor:pointer; font:12px/1.4 inherit; }
.cg-search-row:last-child { border-bottom:0; }
.cg-search-row:hover { background:var(--bd); }
.cg-search-doc { margin-left:auto; opacity:.6; font-size:11px; white-space:nowrap; }
      ...
      function candidates(q: string, cb: any) {
        ...
        if (srvSearch) {
          fetch("/_search?q=" + encodeURIComponent(q))
            ...
            .then(function (a) { cb(a); }).catch(function () { cb([]); });
        } else {
          withIndex(function (rows: any) {
            var out = [];
            for (var i = 0; i < rows.length && out.length < 50; i++) {
              if (String(rows[i][0]).toLowerCase().indexOf(q) >= 0) out.push({ name: rows[i][0], doc: rows[i][1], id: rows[i][2] });
            }
            cb(out);
          });
        }
      ...
      searchBox.addEventListener("input", function () {
        ...
        candidates(qv, function (list: any) {
          ...
          list.forEach(function (c: any) {
            ...
            var dc = document.createElement("span"); dc.className = "cg-search-doc";
            dc.textContent = String(c.doc).replace(/\.geml$/, "").replace(/--/g, "/");
            row.appendChild(nm); row.appendChild(dc);
