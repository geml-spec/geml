...
// `style-frame-cycle` 抓的是另一张图 —— style-frame 的包含关系（设计 §12.4）——
// 区域装区域可以成环，那张图有环检测；状态管道这一条依然成立。

export type StyleDiagnosticCode =
  | "style-selector-unsupported"
  | "style-ambiguous-rule"
  | "style-unmatched-rule"
  | "style-unknown-state"
  | "style-unknown-screen"
  | "style-unmatched-producer"
  | "style-unknown-value-source"
  | "style-unknown-interaction"
  | "style-unknown-token"
  | "style-reserved-name"
  | "style-unknown-component"
  | "style-unknown-handler"
...
export const STYLE_SEVERITY: Record<StyleDiagnosticCode, StyleSeverity> = {
  ...
  "style-unknown-interaction": "error",
  // `{{key}}` 指到本样式表 meta 里没有的键。和核心 §4 的 unknown-metadata-reference
  // 同一判断：单一事实来源的引用悬空了就该响，静默代换成空串会让整页悄悄掉色。
  "style-unknown-token": "error",
  // 一个名字同时是部件名和块类型名。不拒绝、照块类型匹配，只把另一种读法说出来 ——
  // 硬错误会让一个类型叫 `link` 的块用类型名根本选不到，而选择器该尽量命中。
  "style-reserved-name": "warning",
  "style-missing-attribute": "error",
  "style-unmatched-rule": "warning",
  "style-unmatched-producer": "warning",
  "style-unknown-component": "warning",
  "style-unknown-handler": "warning",
  ...
  "style-unknown-frame": "error",
  "style-screen-nested": "error",
  "style-frame-cycle": "error",
  // 嵌套有上限，这是安全边界：样式表是不可信输入（§9），一万个 frame 串成的链没有环，
  // 却不能让宿主渲染一万层盒子。和 embed 的深度上限同一个理由。
  "style-frame-too-deep": "error",
  // 声明了没人引用：我们忽略了作者写下的东西，该说出来 —— 和 unmatched-rule 同性质。
  "style-unused-frame": "warning",
