...
test("detect: nested tsconfig in a Java monorepo -> scip job carries the tsconfig's dir (flink shape)", () => {
  const fx = fixture({
  ...
  assert.equal(jobs[0].indexer, "scip");
  assert.equal(jobs[0].project, "web/dashboard", "scip must run inside the tsconfig project, not the repo root");
  assert.equal(jobs[0].signal, "web/dashboard/tsconfig.json", "the plan line names WHERE TypeScript was detected");
  assert.equal(jobs[0].inferTsconfig, undefined, "a real tsconfig exists — no inference");
  assert.equal(jobs[1].gemlLang, "JAVASRC");
  rmSync(fx, { recursive: true, force: true });
});

test("detect: a root tsconfig subsumes nested ones -> a single scip job at '.'", () => {
  const fx = fixture({ "tsconfig.json": "{}", "src/i.ts": "x", "pkg/a/tsconfig.json": "{}", "pkg/a/i.ts": "y" });
  const jobs = detectLanguages(fx);
  assert.equal(jobs.length, 1);
  assert.equal(jobs[0].project, ".");
  assert.equal(jobs[0].signal, "tsconfig.json");
  rmSync(fx, { recursive: true, force: true });
});

test("detect: sibling tsconfig projects -> one scip job each (sorted); a project inside a kept one is pruned", () => {
  const fx = fixture({
    "apps/b/tsconfig.json": "{}", "apps/b/i.ts": "y",
    "apps/a/tsconfig.json": "{}", "apps/a/i.ts": "x",
    "apps/b/sub/tsconfig.json": "{}", "apps/b/sub/i.ts": "z",
  });
  const jobs = detectLanguages(fx);
  assert.deepEqual(jobs.map((j) => j.project), ["apps/a", "apps/b"], "apps/b/sub is covered by apps/b");
  rmSync(fx, { recursive: true, force: true });
});

test("detect: JS/TS by extension only (no tsconfig anywhere) -> root job with --infer-tsconfig", () => {
  const fx = fixture({ "lib/util.js": "module.exports = 1;" });
  const jobs = detectLanguages(fx);
  assert.equal(jobs.length, 1);
  assert.equal(jobs[0].indexer, "scip");
  assert.equal(jobs[0].project, ".");
  assert.equal(jobs[0].inferTsconfig, true, "scip-typescript exits 1 without a tsconfig unless it infers one");
  rmSync(fx, { recursive: true, force: true });
});

test("indexerCommand: scip job with a project dir -> cwd inside it, per-project .scip filename", () => {
  const cmd = indexerCommand({ indexer: "scip", project: "web/dashboard" },
    { root: "/r", buildDir: "/r/.geml-code-graph/_build", scriptPath: "/x/joern-export.sc" });
  assert.equal(cmd.cwd.replace(/\\/g, "/"), "/r/web/dashboard", "scip runs in the tsconfig's directory");
  assert.equal(basename(cmd.raw), "index-web__dashboard.scip", "raw name is project-unique");
  assert.equal(cmd.argv.at(-1), cmd.raw, "the --output value IS the adapter raw");
  assert.ok(!cmd.argv.includes("--infer-tsconfig"));
});

test("indexerCommand: inferTsconfig scip job passes --infer-tsconfig, root cwd, default raw name", () => {
  const cmd = indexerCommand({ indexer: "scip", project: ".", inferTsconfig: true },
    { root: "/r", buildDir: "/r/.geml-code-graph/_build", scriptPath: "/x/joern-export.sc" });
  assert.ok(cmd.argv.includes("--infer-tsconfig"));
  assert.equal(cmd.cwd, "/r");
  assert.equal(basename(cmd.raw), "index.scip");
...
test("build.mjs auto: one indexer failing does not sink the others (partial map + warning, exit 0)", () => {
  ...
  assert.match(outText, /warning: .*continuing with the rest/, "partiality is called out, not silent");
